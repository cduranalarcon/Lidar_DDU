...
def Lidar_space_time2(date = "2017.02.04",space = 13, timee = 60):
    ...
    #mat_90par_std = np.zeros(shape=(nt,2623))
    #mat_10par_std = np.zeros(shape=(nt,2623))
    #mat_per_std = np.zeros(shape=(nt,2623))
    ...
    if os.path.exists(path1):
        ...
        for t in times:
            ...
            if np.size(pix) > 0 : 
                ...
                try:            
                    for ppix in pix[0]:
                        ...
                        par90sum.append(list(par90))
                        par10sum.append(list(par10))
                        persum.append(list(per))
                        npix = npix + 1. 
            ...
            if npix > 0:
                ...
                mat_90par[t_count,:] = np.nanmean(np.array(par90sum),axis = 0)
                mat_10par[t_count,:] = np.nanmean(np.array(par10sum),axis = 0)
                mat_per[t_count,:] = np.nanmean(np.array(persum),axis = 0)
                #mat_90par_std[t_count,:] = np.nanstd(np.array(par90sum),axis = 0)
                #mat_10par_std[t_count,:] = np.nanstd(np.array(par10sum),axis = 0)
                #mat_per_std[t_count,:] = np.nanstd(np.array(persum),axis = 0)
            ...
            if np.size(pix) > 0 : 
                ...
                try:            
                    for ppix in pix[0]:
                        ...
                        par90,par10,per = read_lidarfile(filename)
                        par90sum.append(par90)
                        par10sum.append(par10)
                        persum.append(per)
                        npix = npix + 1. 
                except:
...
#    aac3=[]
    ...
    matPar90_std = np.zeros(shape=[np.shape(mat_90par)[0],newbins])
    matPar10_std = np.zeros(shape=[np.shape(mat_90par)[0],newbins])
    matPer_std = np.zeros(shape=[np.shape(mat_90par)[0],newbins])
...
#    matPerc = np.zeros(shape=[np.shape(mat_90par)[0],newbins])
    ...
    for htime in range(0,np.shape(mat_90par)[0]):#np.linspace(1,24,24*2-1):
        ...
        #VPL90_std = copy.copy((mat_90par_std)[htime,:])#[int(htime*(nt-1)/24),:])
        #VPL10_std = copy.copy((mat_10par_std)[htime,:])#[int(htime*(nt-1)/24),:])
        #VPLper_std = copy.copy((mat_per_std)[htime,:])#[int(htime*(nt-1)/24),:])
        ...
        VPL90_2_std = np.zeros(shape=newbins)
        VPL10_2_std = np.zeros(shape=newbins)
        VPLper_2_std = np.zeros(shape=newbins)
        ...
        for i in range(newbins):
            ...
            VPL90_2_std[i] = np.nanstd(VPL90[i*new_res:(i+1)*new_res])
            VPL10_2_std[i] = np.nanstd(VPL10[i*new_res:(i+1)*new_res])
            VPLper_2_std[i] = np.nanstd(VPLper[i*new_res:(i+1)*new_res])
        ...
        matPer[htime,:]=Pper
        matPar90_std[htime,:]=copy.copy(VPL90_2_std)
        matPar10_std[htime,:]=copy.copy(VPL10_2_std)    
        matPer_std[htime,:]=copy.copy(VPLper_2_std)    
            
    return[matPar90,matPar10,matPer, r,matPar90_std,matPar10_std,matPer_std]                
